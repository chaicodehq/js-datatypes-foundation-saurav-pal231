...
export function roundFare(amount, decimalPlaces) {
  // Your code here
  if (typeof amount !== "number"|| Number.isInteger(decimalPlaces) === false || decimalPlaces < 0) return "";
...
export function getDistanceDifference(from, to) {
  // Your code here
  const startLoc = parseInt(from);
  const endDest = parseInt(to);
  if (Number.isNaN(startLoc)|| Number.isNaN(endDest)) return -1;
  return Math.abs(endDest-startLoc);
}
